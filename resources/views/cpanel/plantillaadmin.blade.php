...
<body class="bg-slate-100 font-sans text-slate-800 h-screen flex flex-col overflow-hidden">
    ...
    <!-- Se eleva el z-index a 50 para asegurar que siempre quede visible sobre cualquier menú desplegable -->
    <header class="bg-white border-b border-slate-200 px-4 md:px-6 h-16 flex justify-between items-center shrink-0 z-50 relative shadow-2xs">
        <div class="flex items-center space-x-3">
            ...
            <div class="flex items-center space-x-2">
                ...
                <div>
                    ...
                    <p class="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-0.5  xs:block">Panel de Administración Central</p>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center space-x-3">
            <div class="text-right hidden sm:block">
                ...
                <p class="text-[10px] text-rose-700 font-semibold uppercase font-mono tracking-wider">{{ auth()->user()->username ?? 'ADMIN' }}</p>
            </div>
    ...
    <div class="flex-1 flex overflow-hidden relative">
        ...
        <!-- CAPA OSCURA DE FONDO (z-30 para quedar justo por debajo del Aside móvil) -->
        ...
        <!-- BARRA LATERAL (SIDEBAR ENCAJADO) -->
        <!-- Se cambió top-0 por top-16 en móvil y h-screen por h-[calc(100vh-4rem)] para que no tape la Navbar institucional -->
        <aside id="sidebar-menu" class="fixed md:static top-16 bottom-0 left-0 w-64 bg-slate-900 text-slate-400 flex flex-col h-[calc(100vh-4rem)] md:h-full shrink-0 border-r border-slate-800 z-40 transform -translate-x-full md:translate-x-0 transition-transform duration-300 ease-in-out justify-between">
            ...
            <!-- Bloque de Enlaces (Se normaliza el padding en móvil) -->
            <div class="p-4 space-y-6 overflow-y-auto flex-1 custom-scrollbar">
                <!-- Control de Accesos -->
                <div>
                    <span class="text-[9px] font-bold text-slate-500 uppercase tracking-widest px-3 block mb-2 tracking-wider">Control de Accesos</span>
                    <nav class="space-y-1">
                        <a href="{{ route('usuarios.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold transition-colors {{ request()->routeIs('usuarios.index') ? 'bg-[#841B44] text-white shadow-xs' : 'hover:bg-slate-800 hover:text-slate-200' }}">
                ...
                <!-- Estructura Escolar -->
                <div>
                    <span class="text-[9px] font-bold text-slate-500 uppercase tracking-widest px-3 block mb-2 tracking-wider">Estructura Escolar</span>
                    <nav class="space-y-1"> 
                        <a href="{{ route('AdAlumnos.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold transition-colors {{ request()->routeIs('AdAlumnos.index') ? 'bg-[#841B44] text-white shadow-xs' : 'hover:bg-slate-800 hover:text-slate-200' }}">
                            <span class="material-icons-round text-sm">school</span> Alumnos y Matrícula
                        </a>
                        <a href="{{ route('docentes.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold transition-colors {{ request()->routeIs('docentes.index') ? 'bg-[#841B44] text-white shadow-xs' : 'hover:bg-slate-800 hover:text-slate-200' }}">
