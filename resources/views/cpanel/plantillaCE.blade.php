        ...
        <div class="flex items-center space-x-3 md:space-x-4">
            ...
            <div class="flex items-center space-x-3">
                <div class="w-10 h-10 rounded-xl bg-custom-primary flex items-center justify-center shadow-xs">
                    <span class="material-icons-round text-2xl text-white">badge</span>
                </div>
                <div>
                    <h1 class="text-lg md:text-xl font-black tracking-wider leading-none text-custom-primary">SUIE</h1>
                    <p class="text-[10px] md:text-xs text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest mt-0.5">Control Escolar y Matrícula</p>
                </div>
            </div>
        </div>
            ...
            <div class="relative" id="user-menu-container">
                ...
                <div id="user-dropdown-menu" 
                    ...
                    <div class="px-3 py-2.5 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-100 dark:border-slate-700/60">
                        ...
                        <p class="text-xs font-black text-slate-900 dark:text-slate-100 mt-0.5 truncate">{{ auth()->user()->username ?? 'Control Escolar' }}</p>
    ...
    <div class="flex-1 flex overflow-hidden relative">
        ...
        <aside id="sidebar-menu" 
            ...
            <div class="p-4 space-y-6 overflow-y-auto flex-1">
                ...
                <!-- SECCIÓN 1: ADMINISTRACIÓN MATRICULAR -->
                <div>
                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-3 block mb-2.5">Administración Matricular</span>
                    <nav class="space-y-1">
                        <a href="{{ route('alumnos.index') }}" 
                           class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-bold transition-all {{ request()->routeIs('alumnos.*') ? 'bg-custom-primary text-white shadow-xs' : 'hover:bg-slate-800 dark:hover:bg-slate-900 hover:text-slate-100 text-slate-300' }}">
                            <span class="material-icons-round text-base">groups</span>
                            <span>Alumnos Inscritos</span>
                        </a>
                        <a href="{{ route('grupos.index') }}" 
                           class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-bold transition-all {{ request()->routeIs('grupos.*') ? 'bg-custom-primary text-white shadow-xs' : 'hover:bg-slate-800 dark:hover:bg-slate-900 hover:text-slate-100 text-slate-300' }}">
                            <span class="material-icons-round text-base">grid_view</span>
                            <span>Grupos y Secciones</span>
                        </a>
                        <a href="{{ route('materias.index') }}" 
                           class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-bold transition-all {{ request()->routeIs('materias.*') ? 'bg-custom-primary text-white shadow-xs' : 'hover:bg-slate-800 dark:hover:bg-slate-900 hover:text-slate-100 text-slate-300' }}">
                            <span class="material-icons-round text-base">auto_stories</span>
                            <span>Materias</span>
                        </a>
                        <a href="{{ route('cargas.index') }}" 
                           class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-bold transition-all {{ request()->routeIs('cargas.*') ? 'bg-custom-primary text-white shadow-xs' : 'hover:bg-slate-800 dark:hover:bg-slate-900 hover:text-slate-100 text-slate-300' }}">
                ...
                <!-- SECCIÓN 2: TRÁMITES Y CERTIFICACIÓN -->
                <div>
